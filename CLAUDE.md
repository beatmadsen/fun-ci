...
- A run's status follows from its stage rows, settled in one statement each time a stage ends, so the foreground pipeline and the forked slow suite can't overwrite each other's verdict: `test/acceptance/test_run_status_in_any_order.rb`, `test/integration/test_run_status.rb`.
- Each milestone a run reaches is one event in the order reached, and the header plays each milestone's scene from its own pool, queued, each to its end, then rests on the latest outcome: `test/unit/console/test_milestone_events.rb`, `renderer/tests/suite/header_queue.rs`, `renderer/tests/suite/scene_pools.rs`, `renderer/tests/suite/header_rest.rs`.
...
- Scene and art code (`renderer/src/scenes/`, `renderer/src/art/`) uses `art::math::Portable` (`sine`, `exponential`, ...) instead of `f64::sin` and the like: those call the platform's maths library, whose last bits differ between macOS and Linux, and the header digests recorded on one fail on the other (`tests/suite/portable_maths.rs`). Check a snapshot change on Linux with the pinned Rust in Docker before trusting it.
