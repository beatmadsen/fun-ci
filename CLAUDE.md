...
- What the renderer draws for each scenario in `contract/scenarios/` matches its reviewed snapshot in `renderer/tests/suite/snapshots/`, frame by frame (characters, and each cell's colours and attributes); a deliberate change to the TUI is a snapshot diff, accepted with `cargo insta review` and committed with the change: `renderer/tests/suite/snapshots.rs`.
