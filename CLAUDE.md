...
- `lib/fun_ci/pipeline/` -- running a pipeline. `TriggerCommand` parses `fun-ci trigger`; `Trigger` validates the project, runs lint and build in threads, forks the slow suite (`BackgroundFork`) and runs the fast suite; `StageRunner` runs one stage through `CommandExecutor` (an injected runner, or `ProcessRunner` with its budget); `StalePipelineCanceller` cancels an older run on the same branch; `PipelineForker` backs `--no-validate`.
...
- `test/unit/`, `test/acceptance/`, `test/integration/` (SQLite, filesystem), `test/integration/process/` (real processes and git), `test/support/` (guards, scanners, test kits), `features/` (Cucumber).

Seams tests use in place of the real thing:
- `Trigger.new(project:, commit:, io:, seams:)` and `StageRunner.new(commit_hash:, stdout:, seams:)` take a `Pipeline::Seams` (`lib/fun_ci/pipeline/trigger_params.rb`): `command_runner` (stage processes), `background_launcher` (`nil` is the real fork), `commit_validator` (`git cat-file`), `recorder`, `time_budgets`. Each left out gets the real thing; `test/support/trigger_test_kit.rb` builds one the way tests need it.
...
- macOS scans every freshly written executable on its first exec (130-250 ms), so tests that write and run stage scripts are slow there. `test_hook_script_invocation.rb` is left out of the mutation lane for that reason: its Trigger mutants overran mutineer's 10 s cap.
