...
#   /out/v2.bundle         commits the loop landed on top of the host's v2
...
git clone --quiet --branch v2 /src /work/fun-ci
cd /work/fun-ci
...
base=$(git rev-parse origin/v2)
if [ "$(git rev-parse v2)" != "$base" ]; then git bundle create /out/v2.bundle "$base..v2"; fi
