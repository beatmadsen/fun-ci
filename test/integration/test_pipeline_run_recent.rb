...
class TestPipelineRunRecent < Minitest::Test
  ...
  def test_should_list_recent_runs_ordered_by_newest_first
    ...
    id1 = FunCi::Persistence::PipelineRun.create(@db, commit_hash: "aaa", branch: "main")
    FunCi::Persistence::PipelineRun.create(@db, commit_hash: "bbb", branch: "main")
    id3 = FunCi::Persistence::PipelineRun.create(@db, commit_hash: "ccc", branch: "main")
    # When we list recent runs
    runs = FunCi::Persistence::PipelineRun.recent(@db)
  ...
  def test_should_limit_recent_runs
    ...
    5.times { |i| FunCi::Persistence::PipelineRun.create(@db, commit_hash: "hash#{i}", branch: "main") }
    # When we list recent with limit 3
    runs = FunCi::Persistence::PipelineRun.recent(@db, limit: 3)
