...
class TestProcessRunner < Minitest::Test
  ...
  # outcome doesn't depend on timing. A lost kill fails at the deadline.
  ...
  def test_reports_the_process_it_started
    started = []
    _, status, = execute("true", 30) { |pid| started << pid }
  ...
  def test_the_command_starts_only_after_the_caller_has_its_pid
    Dir.mktmpdir do |dir|
      ...
      seen = []
      execute("touch #{marker}", 30) { seen << File.exist?(marker) }
  ...
  def test_seams_without_a_runner_run_the_command_for_real
    _, status, = within_deadline { FunCi::Pipeline::Seams.new.executor(Dir.tmpdir).call("sh -c 'exit 4'", 30) }
  ...
  def run_command(script, budget) = execute("sh -c '#{script}'", budget)

  def execute(command, budget, &)
    within_deadline { Host.new.run_process_with_timeout(command, budget, &) }
  end
end
