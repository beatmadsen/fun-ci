...
class TestScreenBoard < Minitest::Test
  ...
  def test_should_highlight_selected_row
    ...
    assert_match(/>\s*row0/, FunCi::Tui::Ansi.strip(output.string), "Selected row should have cursor marker")
  end
  ...
  def test_should_not_show_cursor_when_nil
    ...
    rows = ["  row0  data"]
    screen.render_board(rows, cursor_index: nil)
    refute_match(/>/, FunCi::Tui::Ansi.strip(output.string), "No cursor marker when cursor_index is nil")
  end
end
