...
class TestMilestoneEvents < Minitest::Test
  ...
  def test_should_order_lint_and_build_by_when_they_finished
    names(a_run)

    assert_equal %w[build_passed lint_passed], names(a_run(lint: done("10:00:05"), build: done("10:00:02")))
  end

  def test_should_put_lint_first_when_lint_and_build_finish_together
    names(a_run)

    assert_equal %w[lint_passed build_passed], names(a_run(build: done, lint: done))
  end
  ...
  def test_should_send_every_milestone_skipped_between_polls_in_order
    ...
    assert_equal %w[lint_passed build_passed fast_passed run_passed],
                 names(a_run(status: "completed", lint: done, build: done, fast: done, slow: done))
  end
  ...
  def done(time = "10:00:00") = { status: "completed", completed_at: "2026-09-26T#{time}Z" }
