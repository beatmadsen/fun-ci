...
class TestAnimationLifecycle < Minitest::Test
  ...
  def test_exposes_type_run_id_and_stage
    anim = FunCi::Tui::Animation.new(type: :success, run_id: 42, stage: "slow")
  ...
  def test_rejects_unknown_type
    assert_raises(ArgumentError) do
      FunCi::Tui::Animation.new(type: :unknown, run_id: 1)
    end
  end
end
...
class TestAnimationFrameCounts < Minitest::Test
  def test_failure_has_40_frames
    anim = FunCi::Tui::Animation.new(type: :failure, run_id: 1)
    assert_equal 40, anim.total_frames
  end

  def test_success_has_40_frames
    anim = FunCi::Tui::Animation.new(type: :success, run_id: 1)
    assert_equal 40, anim.total_frames
  end

  def test_stage_pass_has_3_frames
    anim = FunCi::Tui::Animation.new(type: :stage_pass, run_id: 1)
    assert_equal 3, anim.total_frames
  end

  def test_timeout_has_4_frames
    anim = FunCi::Tui::Animation.new(type: :timeout, run_id: 1)
...
class TestAnimationPriority < Minitest::Test
  ...
  def test_failure_outranks_success
    fail_anim = FunCi::Tui::Animation.new(type: :failure, run_id: 1)
    success_anim = FunCi::Tui::Animation.new(type: :success, run_id: 1)
    assert fail_anim.priority > success_anim.priority
  end

  def test_timeout_outranks_success
    timeout_anim = FunCi::Tui::Animation.new(type: :timeout, run_id: 1)
    success_anim = FunCi::Tui::Animation.new(type: :success, run_id: 1)
