...
class TestStalePipelineCancellerCancel < Minitest::Test
  ...
  def test_should_mark_running_pipeline_as_cancelled_when_process_is_dead
    ...
    old_run = FunCi::Persistence::PipelineRun.find(@db, run_id)
  ...
  def test_should_kill_and_cancel_running_pipeline_when_process_is_alive
    ...
    run_id = FunCi::Persistence::PipelineRun.create(@db, commit_hash: "abc1234", branch: "main")
    FunCi::Persistence::PipelineRun.update_status(@db, run_id, "running")
    FunCi::Persistence::PipelineRun.store_pid(@db, run_id, 12345)
    stdout = StringIO.new
